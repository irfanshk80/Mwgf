...
class maw_claim(osv.osv):
    ...
    _columns = {
        ...
        'pnd_no':fields.char('PND Number', size=64, select=True,track_visibility='onchange'),
        'ewallet_account':fields.char('eWallet Account', size=64, select=True,track_visibility='onchange'),
        'plate_no_head':fields.char('Plate No Prefix', size=64, select=True,track_visibility='onchange'),
        'plate_no_tail':fields.char('Plate No Suffix', size=64, select=True,track_visibility='onchange'),
        'plate_no_full':fields.function(_plate_no_full,type="char",size=64,store=True,string="Plate No"),
        'plate_type':fields.many2one("maw.plate.type", 'Plate Type',track_visibility='onchange'),
        'other_plate_type':fields.char('Other Plate Type', size=64, select=True),
        'staff_id_head':fields.char('Staff ID Head', size=64, select=True,track_visibility='onchange'),
        'staff_id_tail':fields.char('Staff ID Tail', size=64, select=True,track_visibility='onchange'),
        'staff_id_full':fields.function(_staff_id_full,type="char",size=64,store=True,string="Staff ID"),
        'staff_name':fields.char('Staff Name', size=64, select=True,track_visibility='onchange'),
    ...
    def search(self, cr, user, args, offset=0, limit=None, order=None, context=None, count=False):
        if context.get('order_by',False):
            order = context.get('order_by',False)
        return super(maw_claim, self).search(cr, user, args, offset=offset, limit=limit, order=order, context=context, count=count)
